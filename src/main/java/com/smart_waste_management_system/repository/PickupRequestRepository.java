...
public interface PickupRequestRepository extends JpaRepository<PickupRequest, Integer> {
    ...
    @Query("SELECT p FROM PickupRequest p WHERE p.user.user_id = :userId")
    List<PickupRequest> findByUserId(@Param("userId") int userId);

    @Query("SELECT p FROM PickupRequest p WHERE p.category.category_id = :categoryId")
    List<PickupRequest> findByCategoryId(@Param("categoryId") int categoryId);

    @Query("SELECT p FROM PickupRequest p WHERE p.status = :status")
    List<PickupRequest> findByStatus(@Param("status") String status);
